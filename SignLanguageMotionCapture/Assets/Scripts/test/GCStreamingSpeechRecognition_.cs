...
namespace FrostweepGames.Plugins.GoogleCloud.StreamingSpeechRecognition
{
    public class GCStreamingSpeechRecognition_ : MonoBehaviour
    {
		...
		public void StartStreamingRecognition(Config.RecognitionSettings recognitionSettings)
		{
			if(!_initialized)
			{
            ...
            _currentRecognitionSettings = recognitionSettings;
		...
		private void FinishCleaningAfterStreamingRecognition()
        {
		...
		private async void RunStreamingRecognition()
		{
			...
			if (_currentRecognitionSettings.alternativeLanguageCodes != null)
			{
				foreach (var item in _currentRecognitionSettings.alternativeLanguageCodes)
				{
					recognitionConfig.AlternativeLanguageCodes.Add(item.Parse());
				}
			}
